...
# Total observation time for the waterfall in seconds.
# The sweep (1.5-3.5 GHz) repeats continuously for this duration.
# Each complete sweep = one column of the waterfall.
# At N_BLOCKS=100 and STEP_DIVISOR=32: one sweep ~ 13.5 s
# So TOTAL_TIME_S=300 gives ~22 columns; 600 s gives ~44 columns.
TOTAL_TIME_S = 300          # total waterfall duration (seconds)
...
OUTPUT_PLOT      = "rhino_cw_iq_gain_curve.pdf"
OUTPUT_WATERFALL = "rhino_cw_iq_waterfall.pdf"
...
def run_gain_curve_sweep(settle_time_s=DAC_SETTLE_TIME_S):
    ...
    meta = {
        ...
        "method"        : "continuous get_frame() average ",
...
def run_waterfall(total_time_s=TOTAL_TIME_S):
    """
    Continuous sweep waterfall.

    Axes:
      Y = DAC NCO frequency (1.5 to 3.5 GHz)   — one row per DAC step
      X = time (seconds)                         — one column per complete sweep
      Colour = IQ peak power (dBFS) of the averaged spectrum

    How it works:
      For each column (time slot):
        For each row (DAC frequency step):
          1. Set DAC NCO to this frequency
          2. Call get_frame() N_BLOCKS times (rectangular window)
          3. Average N_BLOCKS frames in linear power domain
          4. Find peak power of the averaged spectrum
          5. Store as waterfall[row, col]
        Repeat until total_time_s is reached.

    Parameters
    ----------
    total_time_s : float — total observation time in seconds

    Returns
    -------
    wf           : np.ndarray[N_STEPS, n_cols]  — peak power per (freq, time)
    sweep_freqs  : np.ndarray[N_STEPS]          — DAC NCO frequencies (GHz)
    time_axis_s  : np.ndarray[n_cols]           — time axis (seconds)
    """
    sweep_freqs_ghz = np.linspace(DAC_START_GHZ, DAC_STOP_GHZ, N_STEPS)

    # Estimate columns from timing
    time_per_step_s  = DAC_SETTLE_TIME_S + (N_BLOCKS * N_FFT / ADC_FS_HZ)
    time_per_sweep_s = N_STEPS * time_per_step_s
    n_cols_est = max(1, int(total_time_s / time_per_sweep_s))

    print("\n" + "="*60)
    print("WATERFALL: Continuous sweep")
    print(f"  Y axis: DAC NCO {DAC_START_GHZ}–{DAC_STOP_GHZ} GHz  ({N_STEPS} rows)")
    print(f"  X axis: time  ({total_time_s:.0f} s total, ~{n_cols_est} columns)")
    print(f"  Each cell: peak power of {N_BLOCKS}-frame averaged spectrum")
    print(f"  Time per sweep: ~{time_per_sweep_s:.1f} s")
    print("="*60)

    # Pre-allocate with estimated size; trim at end
    wf          = np.full((N_STEPS, n_cols_est + 5), np.nan, dtype=np.float32)
    time_axis_s = np.zeros(n_cols_est + 5)

    t0  = time.time()
    col = 0

    while (time.time() - t0) < total_time_s:
        col_start_t = time.time() - t0

        for row, f_ghz in enumerate(sweep_freqs_ghz):
            set_tone(f_ghz, amplitude=DAC_AMPLITUDE)
            time.sleep(DAC_SETTLE_TIME_S)
            spec, freqs, _ = acquire_spectrum(n_blocks=N_BLOCKS)
            peak_pwr, _    = find_peak(spec, freqs)
            if col < wf.shape[1]:
                wf[row, col] = peak_pwr

        time_axis_s[col] = col_start_t
        elapsed = time.time() - t0
        remaining = total_time_s - elapsed
        print(f"  Sweep {col+1:3d} complete  "
              f"t={elapsed:.0f}s  remaining={remaining:.0f}s")
        col += 1

        # Grow array if needed
        if col >= wf.shape[1] - 1:
            extra = np.full((N_STEPS, 10), np.nan, dtype=np.float32)
            wf = np.concatenate([wf, extra], axis=1)
            time_axis_s = np.concatenate([time_axis_s, np.zeros(10)])

    disable_tone()

    # Trim to actual columns collected
    wf          = wf[:, :col]
    time_axis_s = time_axis_s[:col]

    print(f"\n  Waterfall done: {col} sweeps in {time.time()-t0:.1f} s")
    return wf, sweep_freqs_ghz, time_axis_s


def plot_waterfall(wf, sweep_freqs_ghz, time_axis_s):
    """
    Plot the waterfall with:
      X axis = time (seconds)
      Y axis = DAC NCO frequency (GHz)
      Colour = IQ peak power (dBFS)
    """
    if wf is None or wf.size == 0:
        print("  No waterfall data.")
        return

    t_max  = float(time_axis_s[-1]) if len(time_axis_s) > 0 else wf.shape[1]
    f_min  = float(sweep_freqs_ghz[0])
    f_max  = float(sweep_freqs_ghz[-1])

    vmin = float(np.nanpercentile(wf, 2))
    vmax = float(np.nanpercentile(wf, 99))

    fig, ax = plt.subplots(figsize=(14, 7))
    fig.suptitle(
        f"RHINO RFSoC 4x2 — CW Calibration Waterfall\n"
        f"X = time | Y = DAC NCO frequency | Colour = IQ peak power (dBFS)\n"
        f"Step = 1/{STEP_DIVISOR} ({STEP_MHZ:.0f} MHz) | "
        f"N_blocks = {N_BLOCKS} | Amp = {DAC_AMPLITUDE:.1f} | "
    ...
    ax.set_ylabel('DAC NCO frequency (GHz)', fontsize=12)
    plt.colorbar(im, ax=ax, label='IQ Peak Power (dBFS)', shrink=0.8)
    plt.tight_layout()
...
def main():
    ...
    print("architecture — continuous get_frame() averaging")
    ...
    # Waterfall: continuous sweep for TOTAL_TIME_S seconds
    # Y = DAC NCO frequency, X = time, colour = IQ peak power
    wf, wf_freqs_ghz, wf_time = run_waterfall(total_time_s=TOTAL_TIME_S)
    plot_waterfall(wf, wf_freqs_ghz, wf_time)

    # Save waterfall data
    np.savez("rhino_cw_waterfall.npz",
             waterfall=wf,
             sweep_freqs_ghz=wf_freqs_ghz,
             time_axis_s=wf_time)
    print("  Saved rhino_cw_waterfall.npz")
