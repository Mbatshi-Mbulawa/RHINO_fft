...
Description:
-----------
Simplified acquisition script for RHINO experiment based on PhD student specifications:
1. Direct sampling at 300 MHz (no DDC, no decimation)
2. Single spectrum output (FFT squared for spectral power)
3. Time-averaged spectra (e.g., 1 second integration)
4. No separate FM band extraction (handle in post-processing)

PhD Student Requirements:
------------------------
"We should be aiming to capture multiple spectra with each spectra being 
 the mean of acquired spectra in a given amount of time like 1 s."

"Just the output of the FFT thats been squared to get the spectral power"

"We just need a single output of spectra, no need to separate the FM band"

System Architecture:
-------------------
    Horn Antenna → Filters → LNAs → ADC (300 MHz)
                                     ↓
                              FFT (16384 pt)
                                     ↓
                             Square (power)
...
class RHINOConfig:
    ...
    # This gives Nyquist at 150 MHz, covering:
    # - RHINO band: 60-85 MHz (science target)
    # - FM band: 88-108 MHz (contamination, handled in post-processing)
    
    TARGET_SAMPLE_RATE_MHZ = 300.0  # Target ADC sample rate
    DECIMATION_FACTOR = 1            # NO DECIMATION (keep full rate)
    
    # =========================================================================
    # FFT Configuration
    # =========================================================================
    FFT_SIZE = 16384  # PhD student specification
                      # At 300 MHz: Resolution = 18.3 kHz per bin
                      # Covers DC to 150 MHz (Nyquist)
    
    NUMBER_OF_FRAMES = 32  # Hardware averaging per spectrum
                           # Can be adjusted based on FPGA capabilities
    
    WINDOW_TYPE = 'hanning'  # Window function for spectral leakage reduction
    
    # PhD student specification: "FFT squared to get spectral power"
    SPECTRUM_TYPE = 'power'  # Options: 'magnitude', 'power', 'log'
                             # 'power' = |FFT|^2
    
    # =========================================================================
    # Time Averaging Configuration
    # =========================================================================
    # PhD student specification:
    # "multiple spectra with each spectra being the mean of acquired 
    #  spectra in a given amount of time like 1 s"
    
    INTEGRATION_TIME_SECONDS = 1.0  # Time to average spectra (e.g., 1 second)
    
    # Number of individual spectra to average will be calculated based on:
    # - FFT size
    # - Sample rate  
    # - Hardware averaging (NUMBER_OF_FRAMES)
    # This gives: spectra_per_integration ≈ Integration_time / spectrum_cadence
    
    # =========================================================================
    # Data Collection
    # =========================================================================
    ACQUISITION_DURATION = 3600    # Total acquisition time (seconds)
    SPECTRA_PER_FILE = 60          # Save one file every N averaged spectra
                                   # e.g., 60 spectra = 1 minute of data
    
    # =========================================================================
    # Calibration
    # =========================================================================
    CALIBRATION_MODE = 1  # ADC calibration mode
    
    # =========================================================================
    # Data Quality
    # =========================================================================
    ENABLE_MD5_CHECKSUM = True   # Compute checksums for integrity
    
    # =========================================================================
    # Logging
    # =========================================================================
    VERBOSE = True               # Detailed console output
    SAVE_LOCAL_COPY = False      # Save locally before transmission
    LOCAL_DATA_DIR = "./data"    # Local storage directory
...
class RHINOAcquisition:
    """
    Simplified RHINO spectrum acquisition for direct sampling.
    
    Key features:
    - Direct sampling at 300 MHz (no DDC)
    - 16384-point FFT with power output
    - Time-averaged spectra (e.g., 1 second integration)
    - Single output spectrum (DC to 150 MHz)
    """
    
    def __init__(self, overlay_path=None):
        """
        Initialize RHINO acquisition system.
        """
        print("="*70)
        ...
        try:
            ...
            print("        This must run on RFSoC with PYNQ installed")
...
# =============================================================================
# COMMENTED OUT: FM BAND SEPARATION (For future use)
# =============================================================================
"""
PhD student: "no need to separate the FM band as its own thing. 
              We can do stuff like that after"

The code below can be uncommented and integrated if FM band separation
...
# In _configure_system():
freq_per_bin = self.adc_sample_rate / RHINOConfig.FFT_SIZE
self.fm_start_bin = int(FM_START_MHZ * 1e6 / freq_per_bin)
self.fm_end_bin = int(FM_END_MHZ * 1e6 / freq_per_bin)

# In capture method:
spectrum_fm = spectrum[self.fm_start_bin:self.fm_end_bin]
fm_power = np.mean(spectrum_fm)

# Add to metadata:
metadata['fm_mean_power'] = float(fm_power)
"""
